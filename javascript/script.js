...
document.addEventListener('DOMContentLoaded', function () {
  // Definir los números de teléfono
  const recipientNumber1 = '543815079230';
  const recipientNumber2 = '543815341528';

  // Elementos del formulario
  const attendanceRadios = document.querySelectorAll('input[name="attendanceOption"]');
  const cantidadInput = document.getElementById('personasCantidad');
  const alimenticioRadios = document.querySelectorAll('input[name="alimenticioOption"]');

  // Función para actualizar el estado del formulario según asistencia
  function updateFormState() {
    const selectedAttendance = document.querySelector('input[name="attendanceOption"]:checked')?.value;
    if (selectedAttendance === 'NO ASISTIRÉ') {
      // Deshabilitar cantidad y opciones alimenticias
    ...
    } else if (selectedAttendance === 'SI ASISTIRÉ') {
      // Habilitarlos
  ...
  // Asignar updateFormState a cada radio de asistencia
  ...
  function sendMessage(phoneNumber) {
    ...
    let finalMessage = `Hola, mi nombre es *${userName}*`;

    if (attendanceStatus.value === 'SI ASISTIRÉ') {
      const cantidadPersonas = cantidadInput.value.trim();
      if (cantidadPersonas === '' || isNaN(cantidadPersonas) || Number(cantidadPersonas) <= 0) {
        alert('Por favor, indicá cuántas personas asistirán (valor mayor a 0).');
        return;
      }

      // Obtener restricción alimenticia
      const alimenticioSeleccionado = document.querySelector('input[name="alimenticioOption"]:checked');
      let restriccionAlimenticia = 'Ninguna';
      if (alimenticioSeleccionado) {
        switch (alimenticioSeleccionado.id) {
          case 'celiaca':       restriccionAlimenticia = 'Celíac@'; break;
          case 'vegetariana':   restriccionAlimenticia = 'Vegetarian@'; break;
          case 'hipertesion':   restriccionAlimenticia = 'Hipertensión'; break;
          case 'diabetica':     restriccionAlimenticia = 'Diabétic@'; break;
        }
      }

      finalMessage += `, mi restricción alimenticia es: *${restriccionAlimenticia}*, y si *asistiré a la fiesta con ${cantidadPersonas} persona/s*`;
    } else {
      finalMessage += ', *no asistiré*';
    }

    if (userMessage) {
      finalMessage += `\n*Mensaje adicional*: ${userMessage}`;
    }
    ...
    alimenticioRadios.forEach(radio => radio.checked = false);

    // Reset estilos y deshabilitación
    updateFormState();

    // Redirigir a sección 'correo'
